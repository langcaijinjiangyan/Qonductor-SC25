...
def load_qpu_backends_from_dir(
    qpu_dir: str = "qpus",
    node_mapping: dict[str, str] | None = None,
) -> list["QPUBackend"]:
    """Load QPU backends from JSON calibration files instead of Qiskit FakeBackends.

    This replaces :func:`get_fake_backends` when the scheduler should operate
    against the real QPU profiles that correspond to quantum worker nodes.

    Args:
        qpu_dir: Directory containing ``*.json`` QPU profile files.
        node_mapping: Optional ``{qpu_name: k8s_node_name}`` dict for
            node affinity in K8s Job dispatch.

    Returns:
        A list of ``QPUBackend`` instances, sorted by name.
    """
    from src.utils.qpu_backend import load_qpu_backends as _load
    return _load(qpu_dir, node_mapping)
